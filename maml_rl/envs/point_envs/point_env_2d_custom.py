...
class MetaPointEnvCustom(gym.Env):
    """
    ...
    def __init__(self, reward_type='sparse', sparse_reward_radius=2, task = {}, points = None):
        ...
        if points is None:
            ...
            self.corners = [np.array(corner) for corner in zip(center[0]+radius*np.cos(theta), center[1]+radius*np.sin(theta))]
    ...
    def reset(self):
        ...
        """
        self._state = self.np_random.uniform(-0.2, 0.2, size=(2,))
    ...
    def sample_tasks(self, n_tasks):
        return [self.corners[idx] for idx in self.np_random.choice(range(len(self.corners)), size=n_tasks)]
