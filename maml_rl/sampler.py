...
class BatchSampler(object):
    ...
    def sample(self, policy, task, params=None, gamma=0.95, device='cpu'):
        episodes = BatchEpisodes(batch_size=self.batch_size, task=task, gamma=gamma, device=device)
        ...
        while (not all(dones)) or (not self.queue.empty()):
            with torch.no_grad():
                ...
                # pdb.set_trace()
                action_probs_tensor, values = policy(observations_tensor, params[episodes._task_id])
            ...
            episodes.append(observations, actions, rewards, batch_ids, action_probs, values)
